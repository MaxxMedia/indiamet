...
const StatsSection = () => {
  ...
  <p className="text-lg leading-relaxed text-black/80 max-w-8xl mb-10">
          ...
          <Link href="/why-exhibit">
            <button className="rounded-full bg-[#8b1a3a] px-10 py-4 text-white font-semibold text-base transition hover:bg-[#083E82]">
              Why Exhibit
            </button>
          </Link>
        </div>
  ...
  <SectionContainer>
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-14">
      {stats.map((stat, index) => (
        <div key={index}>
          <h3 className="text-[52px] font-bold text-[#8b1a3a] mb-2">
            {stat.value}
          </h3>
