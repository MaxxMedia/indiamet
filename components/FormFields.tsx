...
/* =========================================================
   ...
   FIX: switched every one of these to the gray/white/#8b1a3a
    ...
    return (
        <div className={`flex flex-col gap-1.5 ${className}`}>
            <label className="text-xs font-medium text-gray-700 tracking-wide">
                {label} {required && <span className="text-[#8b1a3a]">*</span>}
            </label>
            {children}
        </div>
    );
    ...
    return (
        <div className="relative">
            ...
            <input
                ...
                className={`w-full ${prefix ? 'pl-12' : 'px-3'} py-2.5 text-sm bg-white border border-gray-300 rounded-md text-gray-900 placeholder:text-gray-400 focus:border-[#8b1a3a] focus:ring-1 focus:ring-[#8b1a3a] outline-none transition`}
            />
        </div>
    );
}
...
export function TextArea({
    ...
    return (
        <textarea
            ...
            rows={rows}
            defaultValue={value === undefined ? defaultValue : undefined}
            className="w-full px-3 py-2.5 text-sm bg-white border border-gray-300 rounded-md text-gray-900 placeholder:text-gray-400 focus:border-[#8b1a3a] focus:ring-1 focus:ring-[#8b1a3a] outline-none transition resize-vertical"
        />
    );
}
...
export function Select({
    ...
    return (
        <select
            ...
            required={required}
            defaultValue={value === undefined ? (defaultValue ?? '') : undefined}
            className="w-full px-3 py-2.5 text-sm bg-white border border-gray-300 rounded-md text-gray-900 focus:border-[#8b1a3a] focus:ring-1 focus:ring-[#8b1a3a] outline-none transition appearance-none"
        >
...
export function RadioCard({ title, price, note, name, defaultChecked = false }: RadioCardProps) {
    return (
        <label className="relative flex flex-col items-center justify-center p-3 border border-gray-300 rounded-lg cursor-pointer hover:border-[#8b1a3a] has-[:checked]:border-[#8b1a3a] has-[:checked]:ring-1 has-[:checked]:ring-[#8b1a3a] has-[:checked]:bg-[#8b1a3a]/5 transition-colors bg-white text-center min-h-[100px]">
            <input
                type="radio"
                name={name}
                defaultChecked={defaultChecked}
                className="absolute top-2 right-2 accent-[#8b1a3a]"
            />
            <span className="text-xs font-bold text-[#8b1a3a] tracking-wider">{title}</span>
