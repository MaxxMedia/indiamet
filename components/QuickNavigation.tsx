          ...
          {quickNavItems.map((item) => (
            <div key={item.id} className="bg-white rounded-2xl p-6 lg:p-8 shadow-lg hover:shadow-xl transition-shadow duration-300 border border-gray-100">
              <div className="flex justify-between items-start mb-6">
                <div className="w-14 h-14 lg:w-16 lg:h-16 rounded-full bg-[#00857C] flex items-center justify-center">
              ...
              <div className="mt-auto">
                <Link 
                  href={item.cta.href}
                  className="block w-full rounded-full bg-[#00857C] text-white text-center font-semibold py-3 px-6 hover:bg-[#0074D9] transition-colors text-sm lg:text-base"
                >
                  {item.cta.text}
                </Link>
              </div>
